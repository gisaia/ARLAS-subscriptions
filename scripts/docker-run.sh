...
function run_manager {
    ...
    docker run --net arlas-subscriptions_default --rm busybox sh -c 'i=1; until nc -w 2 subscriptions-manager 9998; do if [ $i -lt 30 ]; then sleep 1; else exit 1; fi; i=$(($i + 1)); done'
}
