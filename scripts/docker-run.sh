...
function run_manager {
    ...
    docker run --net arlas-subscriptions_default --rm busybox sh -c 'i=1; until nc -w 2 subscriptions-manager 9998; do if [ $i -lt 100 ]; then sleep 1; else exit 1; fi; i=$(($i + 1)); done'
}
...
function run_dummy {
    ...
    docker run --net arlas-subscriptions_default --rm busybox sh -c 'i=1; until nc -w 2 arlas-server 9999; do if [ $i -lt 100 ]; then sleep 1; else exit 1; fi; i=$(($i + 1)); done'
}
...
function run_all {
    ...
    docker run --net arlas-subscriptions_default --rm busybox sh -c 'i=1; until nc -w 2 arlas-server 9999; do if [ $i -lt 100 ]; then sleep 1; else exit 1; fi; i=$(($i + 1)); done'
    echo "===> wait for arlas-subscriptions-manage up and running"
    docker run --net arlas-subscriptions_default --rm busybox sh -c 'i=1; until nc -w 2 arlas-subscriptions-manager 9998; do if [ $i -lt 100 ]; then sleep 1; else exit 1; fi; i=$(($i + 1)); done'
}
